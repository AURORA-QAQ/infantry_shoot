...
static void shoot_bullet_control_self(void);

shoot_control_t shoot_control;          //射击数据

	static int16_t tar_spd=0,fric1_give_current=0,fric2_give_current=0;
	pid_type_def fric_pid;
	fp32 PID[3]={15.0f,2.0f,0.0f},PID_ANGLE[3]={1.0F,0.0F,10.0F};//15 1 0
	static uint8_t time_count_end_flag=0;
...
void shoot_init(void)
{
    ...
    static const fp32 Trigger_speed_pid[3] = {TRIGGER_ANGLE_PID_KP, TRIGGER_ANGLE_PID_KI, TRIGGER_ANGLE_PID_KD};
		static const fp32 Trigger_angle_pid[3] = {TRIGGER_ANGLE_PID_KP_SELF,TRIGGER_ANGLE_PID_KI_SELF,TRIGGER_ANGLE_PID_KD_SELF};
...
//	static int16_t tar_spd=0,spd1=0,spd2=0;
//	pid_type_def fric_pid;
//	fp32 PID[3]={15.0f,1.0f,0.0f};
	PID_init(&fric_pid,PID_POSITION,PID,16000,10000);
...
//判断状态
    ...
    else if (shoot_control.shoot_mode == SHOOT_READY_FRIC)
    {
		...
		else if(shoot_control.shoot_mode == SHOOT_SINGLE_BULLET)
		{
			shoot_bullet_control_self();
//			shoot_control.set_angle = 0.0f;
		}
    ...
    else if (shoot_control.shoot_mode == SHOOT_BULLET)//暂时无用
    {
...
//			shoot_bullet_control_self();
			...
			trigger_motor_turn_back();
			//这两句必须同时出现，后一句像个保险(speedset)
        //设置拨弹轮的拨动速度,并开启堵转反转处理
			//shoot_control.trigger_speed_set
//        shoot_control.trigger_speed_set = CONTINUE_TRIGGER_SPEED;//无用//现在可以进入
//        trigger_motor_turn_back();
    }
    ...
    else
    {
        ...
        //计算拨弹轮电机PID
				/**/
				if(shoot_control.shoot_mode==SHOOT_SINGLE_BULLET){
					PID_calc(&shoot_control.trigger_motor_angle_pid, shoot_control.speed, shoot_control.speed_set);
...
//					shoot_control.given_current = (int16_t)(shoot_control.trigger_motor_angle_pid.out);
//				}else{
//					PID_calc(&shoot_control.trigger_motor_pid, shoot_control.speed, shoot_control.speed_set);
//					shoot_control.given_current = (int16_t)(shoot_control.trigger_motor_pid.out);
//				}
...
//    shoot_fric2_on(shoot_control.fric_pwm2);
		//PID_calc(&shoot_control.trigger_motor_pid,shoot_control.shoot_motor_measure[1],2000);
		...
		
		{
			...
			if(shoot_control.shoot_mode==SHOOT_READY_FRIC){
					//spd2=PID_calc();
			    tar_spd=READY_FRIC_SPEED;//5000for proper
		...
		fric1_give_current=PID_calc(&fric_pid,shoot_control.fric1_motor_measure->speed_rpm,tar_spd);
		fric2_give_current=PID_calc(&fric_pid,shoot_control.fric2_motor_measure->speed_rpm,-tar_spd);
		CAN_cmd_fric(fric1_give_current,fric2_give_current,0,0);
...
//				{
//					static uint8_t flag_to_off=0,flag_to_on=1;
//					if((shoot_control.shoot_rc->rc.s[1]==1)&&(shoot_control.shoot_rc->rc.s[0]==3)){
//							flag_to_off=1;
//							flag_to_on=0;
//							shoot_fric1_on(shoot_control.fric_pwm1);
//							shoot_fric2_on(shoot_control.fric_pwm2);
//					}
//					else if ((shoot_control.shoot_rc->rc.s[1]==3)&&(shoot_control.shoot_rc->rc.s[0]==3)){
//								flag_to_off=0;
//								flag_to_on=1;
//								shoot_fric_off();
//					}
...
//		{
//			static uint8_t flag_to_off=0,flag_to_on=1;
//			if((shoot_control.shoot_rc->rc.s[1]==1)&&(shoot_control.shoot_rc->rc.s[0]==3)&&(flag_to_on)){
//					flag_to_off=1;
//					flag_to_on=0;
//			    shoot_fric1_on(shoot_control.fric_pwm1);
//					shoot_fric2_on(shoot_control.fric_pwm2);
//			}
//			else if ((shoot_control.shoot_rc->rc.s[1]==3)&&(shoot_control.shoot_rc->rc.s[0]==3)&&(flag_to_on)){
//						flag_to_off=0;
//						flag_to_on=1;
//						shoot_fric_off();
//			}
...
static void shoot_set_mode(void)
{
    ...
    else if (switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) && (shoot_control.shoot_rc->key.v & SHOOT_OFF_KEYBOARD) && shoot_control.shoot_mode != SHOOT_STOP)
    {
		...
		//为了防止单发发射优先级过低，将单发发射提前
		if(shoot_control.shoot_mode == SHOOT_SINGLE_BULLET)
		{
			shoot_bullet_control_self();
		}//第二个为原最高优先级
    else if(shoot_control.shoot_mode == SHOOT_READY_FRIC && shoot_control.fric1_ramp.out == shoot_control.fric1_ramp.max_value && shoot_control.fric2_ramp.out == shoot_control.fric2_ramp.max_value)
    {
        shoot_control.shoot_mode = SHOOT_READY_BULLET;//唯一可下手
    }
    else if(shoot_control.shoot_mode == SHOOT_READY_BULLET && shoot_control.key == SWITCH_TRIGGER_ON)
    {
        shoot_control.shoot_mode = SHOOT_READY;//无用
    }
    else if(shoot_control.shoot_mode == SHOOT_READY && shoot_control.key == SWITCH_TRIGGER_OFF)
    {
        shoot_control.shoot_mode = SHOOT_READY_BULLET;//无用
    }
    else if(shoot_control.shoot_mode == SHOOT_READY)//无用
    {
        //下拨一次或者鼠标按下一次，进入射击状态
        ...
        {
            shoot_control.shoot_mode = SHOOT_BULLET;//无用
        }
    }
    else if(shoot_control.shoot_mode == SHOOT_DONE)//无用
    {
        if(shoot_control.key == SWITCH_TRIGGER_OFF)
        {
            ...
            {
                shoot_control.key_time = 0;
                shoot_control.shoot_mode = SHOOT_READY_BULLET;//关闭了检测四轮后，有效
            }
        }
        else
        {
    ...
    if(shoot_control.shoot_mode > SHOOT_READY_FRIC)
    {
        ...
        {
            shoot_control.shoot_mode = SHOOT_CONTINUE_BULLET;//active
        }else if (time_count_end_flag){
						time_count_end_flag=0;
						shoot_control.shoot_mode = SHOOT_SINGLE_BULLET;
				}else if(shoot_control.shoot_mode == SHOOT_CONTINUE_BULLET)
        {//为何不进入，迅速转变为其他的变量值了？
...
static void shoot_feedback_update(void)
{
    ...
    {	
			/**/
				if(shoot_control.rc_s_time&&shoot_control.rc_s_time<2000){
						time_count_end_flag=1;
				}
			/**/
...
//感觉这个函数可能完全是为键鼠服务的shoot_bullet_control
...
static void shoot_bullet_control_self(void)
{
	  ...
	  if (shoot_control.move_flag == 0)
    {
        shoot_control.set_angle = rad_format(shoot_control.angle + PI_FIVE);
        shoot_control.move_flag = 1;
    }
		if (rad_format(shoot_control.set_angle - shoot_control.angle) > 0.05f)
    {
...
////	      shoot_control.trigger_speed_set = TRIGGER_SPEED;
////        trigger_motor_turn_back();
	//只要进入单发发射模式，则一定会满足单发角度时间需求
	//pid角度环，当角度环完毕后再进入ready_bullut?
